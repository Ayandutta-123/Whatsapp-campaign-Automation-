...
export default function EditContactModal({ open, onClose, contact, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await contacts.update(contact.id, form);
  ...
  const phoneParts = splitPhone(form.phone, defaultCode);
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <PhoneInput
            ...
            required
          />
          <p className="text-xs text-gray-400 mt-1">{formatPhoneHint(phoneParts.countryCode)}</p>
        </div>
        <div>
